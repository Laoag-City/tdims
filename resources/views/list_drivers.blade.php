...
@section('content')
					...
					<div class="control is-expanded">
						<div class="select is-fullwidth">
							<select name="valid_year">
								<option value="" {{ request()->input('valid_year') != null ?: 'selected'}}>
									None
								</option>
								@foreach($years as $year)
									<option value="{{ $year->years }}" {{ request()->input('valid_year') != $year->years ?: 'selected'}}>
										{{ $year->years }}
									</option>
								@endforeach
							</select>
	...
	<table class="table is-fullwidth is-bordered is-striped is-hoverable">
		...
		<tbody>
			@foreach($drivers as $driver)
				<tr>
					...
					<td>{{ $driver->id_no }}</td>
					<td class="control_no">{{ $driver->getIDControlNos() }}</td>
						...
						<div class="dropdown is-hoverable is-right">
							...
							<div class="dropdown-menu">
								<div class="dropdown-content">
									...
									<!--commented out unitl when it is needed<a href="{{ url('drivers/' . $driver->driver_id) . '/id-printout-back'}}" class="dropdown-item">Print ID (Back)</a>

									<a href="#" class="dropdown-item remove-driver" data-driver="{{ $driver->driver_id }}">Remove</a>-->
								</div>
							</div>
						</div>
					</td>
		...
		@if($drivers->hasPages())
			<tfoot>
				<tr>
					<th colspan="7">{{ $drivers->appends(['valid_year' => request()->input('valid_year')])->links() }}</th>
				</tr>
			</tfoot>
		@endif
	</table>
